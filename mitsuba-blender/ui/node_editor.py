...
from .material import mi_mat_template_ID

class MITSUBA_MATERIAL_MT_node_tree(bpy.types.Menu):
    bl_idname = 'MITSUBA_MATERIAL_MT_node_tree'
    ...
    def draw(self, context):
        ...
        trees = [(index, tree, icon)
                    ...
                    if tree.bl_idname == 'mitsuba_material_nodes']
        ...
        if not trees:
            ...
            col.label(text="No material node trees available")
            col.operator("mitsuba.material_node_tree_new", text="New Node Tree", icon='ADD')
        ...
        for j, (index, tree, icon) in enumerate(trees):
            ...
            op = col.operator('mitsuba.material_node_tree_set', text=text, icon='MATERIAL')
            op.node_tree_index = index

original_draw = None

# Copied from bl_ui/space_node.py (Blender 2.92.0)
def mi_node_header_draw(panel, context):
    layout = panel.layout
    ...
    ###########################################################################################
    # Specialized Mitsuba code
    elif snode.tree_type == 'mitsuba_material_nodes':
        NODE_MT_editor_menus.draw_collapsible(context, layout)
        ...
        if ob:
            ...
            mi_mat_template_ID(row, mat)

            if mat and not mat.mitsuba.node_tree:
                layout.operator('mitsuba.material_node_tree_new', icon='NODETREE', text='Use Mitsuba Material Nodes')
    # End of specialized Mitsuba code
    ###########################################################################################
...
def mi_draw_switch(panel, context):
    if context.scene.render.engine == 'MITSUBA' and context.space_data.tree_type == 'mitsuba_material_nodes':
        mi_node_header_draw(panel, context)
    else:
...
def register():
    ...
    NODE_HT_header.draw = mi_draw_switch
