...
def _set_bl_mesh_shading(bl_mesh, flat_shading=True, flip_normals=False):
    ...
    if flat_shading:
        ...
        # bl_mesh.calc_normals() # TODO is this needed?
...
def mi_ply_to_bl_shape(mi_context, mi_shape):
    ...
    if not bl_mesh:
        logging.error(f'Cannot load PLY mesh file "{filename}".')
        return None
...
def mi_obj_to_bl_shape(mi_context, mi_shape):
    ...
    if len(bl_meshes) > 1:
        logging.warn('OBJ file containing more than one mesh. Only the first one will be loaded.')
    bl_mesh = bl_meshes[0]
    ...
    end_time = time.time()
    logging.info(f'Loaded OBJ mesh "{mi_shape.id()}". Took {end_time-start_time:.2f}s.')
...
def mi_shape_to_bl_shape(mi_context, mi_shape):
    ...
    if shape_type not in _shape_converters:
        logging.error(f'Mitsuba Shape type "{shape_type}" not supported.')
        return None
