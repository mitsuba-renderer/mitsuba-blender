...
def _get_mi_obj_properties(mi_obj):
    ...
    props = {}
    for prop_name, prop_value in mi.traverse(mi_obj):
        props[prop_name] = dr.slice(prop_value)
    return props
...
def convert_mi_srgb_reflectance_spectrum(mi_obj, default):
    # assert mi_obj.class_().name() == 'SRGBReflectanceSpectrum', mi_obj.class_().name() # TODO is needed?
...
def convert_mi_srgb_emitter_spectrum(mi_obj, default):
    # assert mi_obj.class_().name() == 'SRGBEmitterSpectrum', mi_obj.class_().name()
    obj_props = _get_mi_obj_properties(mi_obj)
