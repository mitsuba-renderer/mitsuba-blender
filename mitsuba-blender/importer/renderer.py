...
def mi_fileformat_to_bl_fileformat(mi_context, mi_file_format):
    if mi_file_format not in _fileformat_values:
        logging.error(f'Mitsuba Film image file format "{mi_file_format}" is not supported.')
        return None
...
def mi_pixelformat_to_bl_pixelformat(mi_context, mi_pixel_format):
    if mi_pixel_format not in _pixelformat_values:
        logging.error(f'Mitsuba Film image pixel format "{mi_pixel_format}" is not supported.')
        return None
...
def mi_componentformat_to_bl_componentformat(mi_context, mi_component_format):
    if mi_component_format not in _componentformat_values:
        logging.error(f'Mitsuba Film image component format "{mi_component_format}" is not supported.')
        return None
...
def apply_mi_path_properties(mi_context, mi_props, bl_props=None):
    bl_engine = mi_context.bl_scene.mitsuba_engine if bl_props is None else bl_props

    def update(params):
        params.max_depth = min(mi_props.get('max_depth', -1), 2147483640)
        params.rr_depth  = min(mi_props.get('rr_depth', 5), 2147483640)
        params.hide_emitters = mi_props.get('hide_emitters', False)

    bl_path_props = getattr(bl_engine.available_integrators, 'path', None)
    if bl_path_props is None:
        logging.error(f'Mitsuba Integrator "path" is not supported.')
        return False
    bl_engine.active_integrator = 'path'
    update(bl_path_props)

    bl_path_props = getattr(bl_engine.viewport_available_integrators, 'path', None)
    bl_engine.viewport_active_integrator = 'path'
    update(bl_path_props)
    ...
    if bl_props is None:
        ...
        bl_max_bounces                = min(mi_props.get('max_depth', 1024), 2147483640)
        bl_renderer.min_light_bounces = min(mi_props.get('rr_depth', 5), 2147483640)
        if bl_max_bounces > 0:
...
def apply_mi_prb_properties(mi_context, mi_props, bl_props=None):
    bl_engine = mi_context.bl_scene.mitsuba_engine if bl_props is None else bl_props
    ...
    bl_prb_props = getattr(bl_engine.available_integrators, 'prb', None)
    if bl_prb_props is None:
        logging.error(f'Mitsuba Integrator "prb" is not supported.')
        return False
    bl_engine.active_integrator = 'prb'
    update(bl_prb_props)

    bl_prb_props = getattr(bl_engine.viewport_available_integrators, 'prb', None)
    bl_engine.viewport_active_integrator = 'prb'
    update(bl_prb_props)
...
def apply_mi_direct_properties(mi_context, mi_props, bl_props=None):
    bl_engine = mi_context.bl_scene.mitsuba_engine if bl_props is None else bl_props
    bl_direct_props = getattr(bl_engine.available_integrators, 'direct', None)
    if bl_direct_props is None:
        logging.error(f'Mitsuba Integrator "direct" is not supported.')
        return False
    bl_engine.active_integrator = 'direct'
    bl_direct_props.hide_emitters = mi_props.get('hide_emitters', False)

    bl_direct_props = getattr(bl_engine.viewport_available_integrators, 'direct', None)
    bl_engine.viewport_active_integrator = 'direct'
    bl_direct_props.hide_emitters = mi_props.get('hide_emitters', False)
    ...
    if bl_props is None:
        bl_renderer = mi_context.bl_scene.cycles
        bl_renderer.progressive = 'PATH'
        bl_max_bounces = mi_props.get('max_depth', 1)
        bl_renderer.max_bounces = bl_max_bounces
        bl_renderer.diffuse_bounces = bl_max_bounces
        bl_renderer.glossy_bounces = bl_max_bounces
        bl_renderer.transparent_max_bounces = bl_max_bounces
...
def apply_mi_moment_properties(mi_context, mi_props, bl_props=None):
    if bl_props is not None:
        ...
        logging.error('Mitsuba Integrator "moment" does not support being nested yet.')
        return False

    mi_renderer = mi_context.bl_scene.mitsuba_engine
    bl_moment_props = getattr(mi_renderer.available_integrators, 'moment', None)
    if bl_moment_props is None:
        logging.error(f'Mitsuba Integrator "moment" is not supported.')
        return False
    ...
    for mi_integrator_props in mi_props_utils.named_references_with_class(mi_context, mi_props, 'Integrator'):
        bl_child_integrator_list.new(name=mi_integrator_props[0])
        bl_child_integrator = bl_child_integrator_list.collection[bl_child_integrator_list.count-1]
        if not apply_mi_integrator_properties(mi_context, mi_integrator_props[1], bl_child_integrator):
            return False

    # Cycles properties
    logging.warn('Mitsuba Integrator "moment" is not supported in Blender Cycles')

    return True

_mi_integrator_properties_converters = {
    'path':                apply_mi_path_properties,
    'prb':                 apply_mi_prb_properties,
    'direct':              apply_mi_direct_properties,
    'moment':              apply_mi_moment_properties,
    # TODO support aov integrator!
}
...
def apply_mi_integrator_properties(mi_context, mi_props, bl_integrator_props=None):
    ...
    if mi_integrator_type not in _mi_integrator_properties_converters:
        logging.error(f'Mitsuba Integrator "{mi_integrator_type}" is not supported.')
        return False
...
def apply_mi_tent_properties(mi_context, mi_props):
    mi_camera = mi_context.bl_scene.camera.data.mitsuba_engine
    bl_box_props = getattr(mi_camera.rfilters, 'tent', None)
    if bl_box_props is None:
        logging.error(f'Mitsuba Reconstruction Filter "tent" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_box_properties(mi_context, mi_props):
    ...
    if bl_box_props is None:
        logging.error(f'Mitsuba Reconstruction Filter "box" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_gaussian_properties(mi_context, mi_props):
    ...
    if bl_box_props is None:
        logging.error(f'Mitsuba Reconstruction Filter "gaussian" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_rfilter_properties(mi_context, mi_props):
    ...
    if mi_rfilter_type not in _mi_rfilter_properties_converters:
        logging.error(f'Mitsuba Reconstruction Filter "{mi_rfilter_type}" is not supported.')
        return False
...
def apply_mi_independent_properties(mi_context, mi_props):
    ...
    if bl_independent_props is None:
        logging.error(f'Mitsuba Sampler "independent" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_stratified_properties(mi_context, mi_props):
    ...
    if bl_stratified_props is None:
        logging.error(f'Mitsuba Sampler "stratified" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_multijitter_properties(mi_context, mi_props):
    ...
    if bl_multijitter_props is None:
        logging.error(f'Mitsuba Sampler "multijitter" is not supported.')
        return False
    # Mitsuba properties
...
def apply_mi_sampler_properties(mi_context, mi_props):
    ...
    if mi_sampler_type not in _mi_sampler_properties_converters:
        logging.error(f'Mitsuba Sampler "{mi_sampler_type}" is not supported.')
        return False
...
def apply_mi_film_properties(mi_context, mi_props):
    ...
    if mi_film_type not in _mi_film_properties_converters:
        logging.error(f'Mitsuba Film "{mi_film_type}" is not supported.')
        return False
