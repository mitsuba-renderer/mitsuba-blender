...
def _eval_mi_bsdf_retro_reflection(mi_context, mi_mat, default):
    ...
    color, pdf = bsdf.eval_pdf(mi.BSDFContext(), si, wo)
    color = dr.slice(color)
    pdf   = dr.slice(pdf)
    if pdf == 0.0:
        return default
...
def mi_wrap_mode_to_bl_extension(mi_context, mi_wrap_mode):
    if mi_wrap_mode == 'repeat':
        ...
        logging.error(f'Mitsuba wrap mode "{mi_wrap_mode}" is not supported.')
        return None
...
def mi_filter_type_to_bl_interpolation(mi_context, mi_filter_type):
    if mi_filter_type == 'bilinear':
        ...
        logging.error(f'Mitsuba filter type "{mi_filter_type}" is not supported.')
        return None
...
def mi_ior_string_to_float(mi_context, mi_ior):
    if mi_ior not in _ior_string_values:
        logging.error(f'Mitsuba IOR name "{mi_ior}" is not supported.')
        return 1.0
...
def mi_microfacet_to_bl_microfacet(mi_context, mi_microfacet_distribution):
    if mi_microfacet_distribution not in _microfacet_distribution_values:
        logging.error(f'Mitsuba microfacet distribution "{mi_microfacet_distribution}" not supported.')
        return 'BECKMANN'
...
def write_mi_float_texture(mi_context, mi_texture, bl_mat_wrap, out_socket_id, default=None):
    ...
    if mi_texture_type not in _float_texture_writers:
        logging.error(f'Mitsuba Texture type "{mi_texture_type}" is not supported.')
        return
...
def write_mi_float_property(mi_context, mi_mat, mi_prop_name, bl_mat_wrap, out_socket_id, default=None, transformation=None):
    ...
    if mi_mat.has_property(mi_prop_name):
        ...
        if mi_prop_type in [mi.Properties.Type.Float, mi.Properties.Type.Long]:
            ...
            ref_id = mi_mat.get(mi_prop_name)
        ...
        logging.error(f'Material "{mi_mat.id()}" does not have property "{mi_prop_name}".')
...
def write_mi_rgb_texture(mi_context, mi_texture, bl_mat_wrap, out_socket_id, default=None):
    ...
    if default is not None:
        default = mi.ScalarColor3f(default)
    mi_texture_type = mi_texture.plugin_name()
...
def write_mi_rgb_srgb_reflectance_spectrum(mi_context, mi_obj, bl_mat_wrap, out_socket_id, default=None):
    reflectance = mi_spectra_utils.convert_mi_srgb_reflectance_spectrum(mi_obj, default)
    if len(bl_mat_wrap.out_node.inputs[out_socket_id].default_value) == 4:
        reflectance = bl_shader_utils.rgb_to_rgba(reflectance)
    bl_mat_wrap.out_node.inputs[out_socket_id].default_value = reflectance
...
def write_mi_rgb_property(mi_context, mi_mat, mi_prop_name, bl_mat_wrap, out_socket_id, default=None, force_rgb=False):
    import mitsuba as mi
    if default is not None:
        default = mi.ScalarColor3f(default)
    if mi_mat.has_property(mi_prop_name):
        ...
        if not force_rgb:
            default = bl_shader_utils.rgb_to_rgba(default)
        bl_mat_wrap.out_node.inputs[out_socket_id].default_value = default
    else:
        logging.error(f'Material "{mi_mat.id()}" does not have property "{mi_prop_name}".')
...
def write_mi_ior_property(mi_context, mi_mat, mi_prop_name, bl_mat_wrap, out_socket_id, default=None):
    ...
    if mi_mat.has_property(mi_prop_name):
        ...
        elif mi_prop_type == mi.Properties.Type.String:
...
def write_mi_bump_and_normal_maps(mi_context, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    if mi_bump is not None:
        ...
        write_mi_float_bitmap(mi_context, mi_bump_textures[0][1], bl_bump_wrap, 'Height', 0.0)
...
def write_mi_principled_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    write_mi_float_property(mi_context, mi_mat, 'specular', bl_principled_wrap, 'Specular IOR Level', 0.5)
    write_mi_rgb_property(mi_context, mi_mat, 'spec_tint', bl_principled_wrap, 'Specular Tint', [1.0, 1.0, 1.0])
    write_mi_float_property(mi_context, mi_mat, 'spec_trans', bl_principled_wrap, 'Transmission Weight', 0.0)
    ...
    write_mi_float_property(mi_context, mi_mat, 'sheen', bl_principled_wrap, 'Sheen Weight', 0.0)
    write_mi_rgb_property(mi_context, mi_mat, 'sheen_tint', bl_principled_wrap, 'Sheen Tint', 0.5)
    write_mi_float_property(mi_context, mi_mat, 'clearcoat', bl_principled_wrap, 'Coat Weight', 0.0)
    write_mi_roughness_property(mi_context, mi_mat, 'clearcoat_gloss', bl_principled_wrap, 'Coat Roughness', 0.03)
...
def write_mi_twosided_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    if mi_child_material_count == 1:
        ...
        write_mi_material_to_node_graph(mi_context, mi_child_materials[0][1], bl_mat_wrap, out_socket_id, is_within_twosided=True, mi_bump=mi_bump, mi_normal=mi_normal)
        return True
        ...
        write_twosided_material(mi_context, bl_mat_wrap, out_socket_id, mi_child_materials[0][1], mi_child_materials[1][1], mi_bump=mi_bump, mi_normal=mi_normal)
        return True
    else:
        logging.error(f'Mitsuba twosided material "{mi_mat.id()}" has {mi_child_material_count} child material(s). Expected 1 or 2.')
        return False
...
def write_mi_dielectric_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    bl_glass.distribution = 'BECKMANN'
...
def write_mi_blend_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    if mi_child_mats_count != 2:
        logging.error(f'Unexpected number of child BSDFs in blendbsdf. Expected 2 but got {mi_child_mats_count}.')
        return False
    write_mi_material_to_node_graph(mi_context, mi_child_mats[0][1], bl_mix_wrap, 'Shader', mi_bump=mi_bump, mi_normal=mi_normal)
    write_mi_material_to_node_graph(mi_context, mi_child_mats[1][1], bl_mix_wrap, 'Shader_001', mi_bump=mi_bump, mi_normal=mi_normal)
    return True
...
def write_mi_conductor_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    bl_glossy.distribution = 'BECKMANN'
...
def write_mi_mask_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    ...
    if mi_child_mats_count != 1:
        logging.error(f'Unexpected number of child BSDFs in mask BSDF. Expected 1 but got {mi_child_mats_count}.')
        return False
    write_mi_material_to_node_graph(mi_context, mi_child_mats[0][1], bl_mix_wrap, 'Shader_001', mi_bump=mi_bump, mi_normal=mi_normal)
    return True
...
def write_mi_bumpmap_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    if mi_bump is not None:
        logging.error('Cannot have nested bumpmap BSDFs')
        return False
    child_mats = mi_props_utils.named_references_with_class(mi_context, mi_mat, 'BSDF')
    assert len(child_mats) == 1

    write_mi_material_to_node_graph(mi_context, child_mats[0][1], bl_mat_wrap, out_socket_id, mi_bump=mi_mat, mi_normal=mi_normal)
    return True

def write_mi_normalmap_bsdf(mi_context, mi_mat, bl_mat_wrap, out_socket_id, mi_bump=None, mi_normal=None):
    if mi_normal is not None:
        logging.error('Cannot have nested normalmap BSDFs')
        return False
    child_mats = mi_props_utils.named_references_with_class(mi_context, mi_mat, 'BSDF')
    assert len(child_mats) == 1

    write_mi_material_to_node_graph(mi_context, child_mats[0][1], bl_mat_wrap, out_socket_id, mi_bump=mi_bump, mi_normal=mi_mat)
    return True
...
def write_mi_material_to_node_graph(mi_context, mi_mat, bl_mat_wrap, out_socket_id, is_within_twosided=False, mi_bump=None, mi_normal=None):
    ...
    if mat_type not in _material_writers:
        logging.warn(f'Mitsuba BSDF type "{mat_type}" not supported. Skipping.')
        write_bl_error_material(bl_mat_wrap, out_socket_id)
        return

    if is_within_twosided and mat_type == 'twosided':
        logging.error('Cannot have nested twosided materials.')
        return
    ...
    if not is_within_twosided and mat_type != 'twosided' and mat_type not in _always_twosided_bsdfs:
        ...
        logging.warn(f'Failed to convert Mitsuba material "{mi_mat.id()}". Skipping.')
        write_bl_error_material(bl_mat_wrap, out_socket_id)

def mi_material_to_bl_material(mi_context, mi_mat, mi_props_id, mi_emitter=None):
    ...
    bl_mat = bpy.data.materials.new(name=mi_props_id)
